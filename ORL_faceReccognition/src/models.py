...
class ResBlock(nn.Module):
    def __init__(self, inchannel, outchannel, stride=1):
        ...
        # 这里定义了残差块内连续的2个卷积层,第一个提取特征，第二个用于进一步处理特征
        ...
        self.shortcut = nn.Sequential()  # 根据输入特征和输出特征的通道数以及步长的关系，判断是否需要对输入特征进行处理，以保持维度一致性
...
class ResNet(nn.Module):
    # 设计基于 ResNet 的思想，使用了残差块来减轻梯度消失问题，同时通过堆叠多个残差块来增加模型的深度和表达能力。模型的最后通过全连接层进行分类预测。
    def __init__(self, ResBlock, num_classes=16):
        super(ResNet, self).__init__()
        self.inchannel = 64
        self.conv1 = nn.Sequential(  # 一个卷积层和一个最大池化层
            # 1 * 100 * 100
            nn.Conv2d(1, 64, kernel_size=3, stride=1, padding=1, bias=False),  # 输入通道数为1，输出通道数为64的卷积层。
    ...
    def forward_once(self, x):  # 模型的前向传播过程，用于处理单个输入
        ...
        out = self.layer3(out)
        out = self.layer4(out)
        out = F.avg_pool2d(out, 3)  # 平均池化层，对输入进行3x3的平均池化操作
